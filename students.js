...
class App extends React.Component {
  ...
  autoRead() {
    db.collection("db_project").onSnapshot((querySnapshot) => {
      var stdlist = [];
      querySnapshot.forEach((doc) => {
        stdlist.push({ id: doc.id, ...doc.data() });
      });
  ...
  insertData() {
    db.collection("db_project").doc(this.state.stdid).set({
  ...
  delete(std) {
    if (window.confirm("ต้องการลบข้อมูล")) {
      db.collection("db_project").doc(std.id).delete();
    }
  }
}
